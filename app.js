...
// TMDB-powered TV episode sidebar: fetches every season + episode for the show,
// grouped under season headers, and clicking one jumps playback.
async function renderTmdbEpisodes(tvItem, server) {
    ...
    try {
        ...
        if (countEl) {
            let total = 0;
            (detail.seasons || []).forEach(s => { if (s) total += (s.episode_count || 0); });
            countEl.textContent = seasons.length + (total ? ` season${seasons.length !== 1 ? 's' : ''} · ${total} ep.` : '');
        }
        if (!seasons.length) { if (loading) loading.style.display = 'none'; return; }

        // Fetch all seasons in parallel, then render them in order.
        ...
        seasonInfos.forEach(({ sn, eps }) => {
            const group = document.createElement('div');
            group.className = 'season-group';
            const cap = sn === 0 ? 'Specials' : `Season ${sn}`;
            const count = eps.length;
            const header = document.createElement('div');
            header.className = 'season-group-title';
            header.innerHTML = `<span class="sg-name">${escapeHtml(cap)}</span><span class="sg-count">${count} ep.</span> <button type="button" class="season-toggle" title="Toggle">▾</button>`;
            // Clicking anywhere on the season header seesaws it open / closed.
            header.onclick = () => group.classList.toggle('collapsed');
            group.appendChild(header);
            const inner = document.createElement('div');
            inner.className = 'ep-list-inner';
            eps.forEach(ep => {
                const div = document.createElement('div');
                div.className = 'ep-play-item';
                div.dataset.season = sn;
                ...
                div.innerHTML = `
                    ...
                    <div class="ep-info">
                        ...
                        <div class="ep-size">S${sn || 0}E${ep.episode_number}${ep.runtime ? ' · ' + ep.runtime + 'm' : ''}</div>
                    </div>`;
                div.onclick = () => {
                    setActiveEpisode(sn, ep.episode_number, list);
                    playTmdbEpisode(tvItem, sn, ep.episode_number, server);
                };
        ...
        // Highlight the currently-playing episode and make sure its season is expanded.
        if (playContext && playContext.item && String(playContext.item.id) === String(tvItem.id)) {
            const curSeason = (playContext.season || 1);
            list.querySelectorAll('.season-group').forEach(g => {
                const firstEp = g.querySelector('.ep-play-item');
                g.classList.remove('collapsed');
                if (firstEp && String(firstEp.dataset.season) === String(curSeason)) firstEp.classList.add('season-current');
            });
            setActiveEpisode(curSeason, playContext.episode || 1, list);
        }
    } catch (e) {
...
function showSearchResults() {
    ['moviesSection','tvShowsSection','myListSection','homeGenres','trendingSection','streamingSection','theatersSection','heroSection'].forEach(id => {
...
document.getElementById('episodeToggleBtn').addEventListener('click', () => { document.getElementById('episodeSelector').classList.toggle('collapsed'); });
...
document.getElementById('logoWrap').addEventListener('click', (e) => {
    ...
    ['trendingSection', 'streamingSection', 'theatersSection', 'homeGenres'].forEach(id => document.getElementById(id).style.display = 'none');
    renderAll();
    window.scrollTo({ top: 0, behavior: 'smooth' });
});
