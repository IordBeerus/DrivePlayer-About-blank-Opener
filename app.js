...
function renderLiveTab(section) {
    if (section === 'trending') {
        ...
        if (st.items.length) renderLiveGrid('manga');
        else if (!liveFed.manga) fetchLive(Array.from({ length: LIVE_PAGES }, (_, i) => `/discover/tv?with_genres=16&with_keywords=210024|287501|282717&page=${i + 1}`), 'tv', 'manga');
    }
}
