...
async function loadPopularContent() {
    ...
    if (existing > 0 && !confirm(`Load a big TMDB library (every genre, movies + shows + anime, newest first) into LuckyFlix? Your existing ${existing} item(s) are kept and duplicates are skipped.`)) return;
    btn.dataset.busy = '1';
    const original = btn.textContent;
    btn.disabled = true;
